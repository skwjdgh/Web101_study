...
class Content {
  ...
  extractContentFromThumbnail(thumbnail, contentId) {
    try {
      ...
      return {
        ...
        title: title,
        category: category,
        content: contentText.replace('...', ''), // 말줄임표 제거
    ...
    const modalContent = `
      <div class="modal" style="width: 800px;">
        ...
        <div class="modal-actions">
          ...
          <button onclick="duplicateContentFromModal(${content.id})" style="background: #6f42c1; color: white;">📋 복사</button>
  ...
  // 모든 콘텐츠 삭제
  deleteAllContent() {
    showConfirm('모든 콘텐츠를 삭제하시겠습니까?', () => {
      showConfirm('정말로 모든 콘텐츠를 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.', () => {
        // 모든 썸네일 제거
  ...
  exportContents() {
    ...
    showAlert('모든 콘텐츠가 성공적으로 내보내기되었습니다!');
  }
...
window.editContent = (contentId) => {
  if (window.contentInstance) {
    window.contentInstance.editContentById(contentId);
  }
};

window.deleteContent = (contentId) => {
  if (window.contentInstance) {
    window.contentInstance.deleteContentById(contentId);
  }
};

window.duplicateContentFromModal = (contentId) => {
  if (window.contentInstance) {
    closeModal();
    window.contentInstance.duplicateContent(contentId);
  }
};
